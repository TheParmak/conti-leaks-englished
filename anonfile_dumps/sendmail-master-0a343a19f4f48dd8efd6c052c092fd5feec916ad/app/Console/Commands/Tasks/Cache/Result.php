...
class Result extends Command
{
     ...
     * @var string
     */
    protected $signature = 'tasks:cache:result';
    ...
    {
        $cache = [];
        if(Storage::exists('result.json')){
            $cache = json_decode(Storage::get('result.json'), true);
        }
        ...
        foreach ($files as $f){
            ...
            if(!isset($cache[$id[0]]) || (isset($cache[$id[0]]) && $size != $cache[$id[0]]['size'])){
                $good_path = storage_path('data/'.$id[0].'/result.email');
                $count_good = exec('wc -l '.escapeshellarg($good_path));
                preg_match('#^\d+#', $count_good, $count_good);

                $bad_path = storage_path('data/'.$id[0].'/result.err');
        ...
        Storage::put('result.json', json_encode($cache));
        sleep(2);
    }
}
