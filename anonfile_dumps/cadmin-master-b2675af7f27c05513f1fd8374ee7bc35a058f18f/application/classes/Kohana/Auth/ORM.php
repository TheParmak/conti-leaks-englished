...
class Kohana_Auth_ORM extends Auth {
	...
	public function auto_login()
	{
		if ($token = Cookie::get('authautologin'))
		{
			...
			{
                // If auto_login get called before initial Request::factory
                if (null === Request::$user_agent && isset($_SERVER['HTTP_USER_AGENT'])) {
    ...
    protected function logLogin(Model_User $user, $isRemembermeLogin = false)
    {
        if ( ! $isRemembermeLogin) {
            // Update users's last login timestamp and logins count
            ...
            // Log to userslogs
            ORM::factory('Userslogs')->createLog3($user, 'Logged in');
        } else {
            // Log to userslogs
            ORM::factory('Userslogs')->createLog3($user, 'Relogged in using &quot;Remember me&quot; token');
        }
        
        // Log to lastlogins
        ORM::factory('User_Lastlogin')->createLastlogin($user, $this->_session->getIp(), $this->_session->getUserAgent(), $isRemembermeLogin);
    }
