...
class Kohana_Session_Database extends Session {
    ...
    public function __construct(array $config = NULL, $id = NULL)
    {
        ...
        if ($this->_gc && mt_rand(0, $this->_gc) === $this->_gc)
        {
        ...
        $this->_session_key = Kohana::$config->load('auth.session_key');
        $this->_user_ip = Arr::get($_SERVER, 'REMOTE_ADDR');
        $this->_user_agent = Arr::get($_SERVER, 'HTTP_USER_AGENT');
    }
    ...
    {
        if ($this->_session_key = $key && $value instanceof Model_Auth_User) {
    ...
    protected function _read($id = NULL)
    {
        if ($id OR $id = Cookie::get($this->_name))
        {
            $result = DB::select(array($this->_columns['contents'], 'contents'))
                ->select([$this->_columns['user_id'], 'user_id'])
                ->from($this->_table)
            ...
            if ($result->count())
            {
                ...
                // Restore user id
                $this->_user_id = $result->get('user_id');
    ...
    protected function _write()
    {
        if ($this->_update_id === NULL)
        {
            ...
            $query = DB::insert($this->_table, $this->_columns)
                ->values(array(':new_id', ':active', ':user_id', ':user_ip', ':user_agent', ':contents'));
        }
        else
        {
            // Update the row
            $query = DB::update($this->_table)
                ...
                ->value($this->_columns['user_ip']    , ':user_ip')
                ->value($this->_columns['user_agent'] , ':user_agent')
        ...
        $query
            ...
            ->param(':user_ip',    $this->_user_ip)
            ->param(':user_agent', $this->_user_agent)
